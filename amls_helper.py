...
def extract_features_labels(images_dir, labels_file, task=1, method='landmark_features', model='hog', sample_times=1):
    ...
    for img_file in os.listdir(images_dir):
        ...
        elif method == 'face_locations':
            img = face_recognition.load_image_file(img_path)
            face_locations = face_recognition.face_locations(img, model=model, number_of_times_to_upsample=sample_times)
            if face_locations:
...
def get_labels(labels_file, task=1):
    ...
    labels = {}
    for idx,line in enumerate(open(labels_file)):
        if idx < 2:
            continue
